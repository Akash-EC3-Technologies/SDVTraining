...
class MqttService:
    ...
    Thin wrapper around paho-mqtt that:
      ...
      - subscribes to vehicle Commands
    """
    ...
    def connect(self) -> None:
        ...
        self.client_id = os.environ.get("MQTT_CLIENT_ID", VEHICLE_ID)
    ...
    def publish_ota_update_status(self, payload: dict[str, Any]) -> None:
        if self.client is None:
            raise RuntimeError("MQTT client not connected")
        topic = f"/vehicles/{self.client_id}/status/ota-update"
        result = self.client.publish(topic, json.dumps(payload), qos=1)
        result.wait_for_publish()

    def subscribe_ota_update_command(self) -> None:
        if self.client is None:
            raise RuntimeError("MQTT client not connected")
        topic = f"/vehicles/{self.client_id}/commands/ota-update"
        result, mid = self.client.subscribe(topic, qos=1)
        if result != mqtt.MQTT_ERR_SUCCESS:
            raise RuntimeError(f"Subscribe failed: {result}")

    def loop_foreever(self):
        if self.client is None:
            raise RuntimeError("MQTT client not connected")
    ...
    def _on_connect(
        ...
        self.subscribe_ota_update_command()
